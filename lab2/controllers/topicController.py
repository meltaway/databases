...
class TopicController(object):
    ...
    def getAll(self, page: int, per_page: int):
        ...
        try:
            page -= 1
            self.db.cursor.execute(f"SELECT {Topic().getKeys()} FROM topics ORDER BY id LIMIT {per_page} OFFSET {page * per_page}")
    ...
    def getById(self, tid):
        ...
        try:
            ...
            self.db.cursor.execute(f"SELECT {item.getKeys()} FROM topics WHERE id = {tid}")
    ...
    def add(self, *args):
        try:
            item: Topic = Topic()
            if len(args) > 0 and isinstance(args[0], Topic):
                item = args[0]
            else:
                item.fill()

            if item.isFull():
                self.db.cursor.execute(f"INSERT INTO topics ({item.getKeys()}) VALUES ({item.getValues()}) RETURNING id as tid; ")
    ...
    def delete(self, tid):
        try:
            ...
            self.db.cursor.execute(f"DELETE FROM topics WHERE id = {tid}")
            self.db.connection.commit()
            return item
        except Exception as err:
    ...
    def update(self, *args):
        try:
            ...
            self.db.cursor.execute(f"UPDATE topics SET {query[:-2]} WHERE id = {item.id}")
            self.db.connection.commit()
            return True
        except Exception as err:
    ...
    def getCount(self):
        try:
            self.db.cursor.execute(f"SELECT COUNT(*) FROM topics")
    ...
    def generateRows(self, entitiesNum: int):
        start = time.time()
        try:
            self.db.cursor.execute(f"INSERT  INTO topics (name, tag_id)"
