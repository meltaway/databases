...
class CUI(object):
    ...
    def __stepController(self, char):
        ...
        if (charCode == 119 or charCode == 97) and self.__currentPos > 1:
            self.__currentPos += -1
        if (charCode == 115 or charCode == 98) and self.__currentPos < upperLimit:
            self.__currentPos += 1
        if charCode == 10 or charCode == 13:
            self.__currentNode.childs[self.__currentPos - 1].on_press()
    ...
    def addMenu(self, title):
        ...
        newNode.append("Return to previous Menu", lambda: self.__goToParent())
        self.__currentNode.append(newNode)
        self.__currentNode = newNode

    def finishMenu(self):
        if self.__currentNode.root is not None:
            self.__goToParent()
